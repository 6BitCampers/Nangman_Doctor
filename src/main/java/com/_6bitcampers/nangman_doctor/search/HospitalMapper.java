...
public interface HospitalMapper {
    ...
    @Select("SELECT * FROM hospital_info WHERE info_description LIKE CONCAT('%', #{keyword}, '%') OR info_name LIKE CONCAT('%', #{keyword}, '%')")
    List<HospitalDto> findByNameContaining(String keyword);

    @Select("SELECT * FROM hospital_info WHERE info_description LIKE CONCAT('%', #{keyword}, '%') OR info_name LIKE CONCAT('%', #{keyword}, '%') ORDER BY info_likecount DESC")
